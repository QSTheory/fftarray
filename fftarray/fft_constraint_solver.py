"""Constraint solver based on `Z3 <https://github.com/Z3Prover/z3>`_ - a theorem prover from Microsoft Research.
...
def _z3_constraint_solver(
    ...
    ----------
    loose_params : List[str]
        If `n` is not explicitly defined it is in general not a natural
        number. Therefore it needs to be rounded up to the next natural number or usually for
        performance reasons to the next power of two. But when the original solution was unique this
        would then violate the other given constraints. But when we simultanously improve another
        parameter to match the increase of `n` we again get a unique solution.
        Here, one or multiple of the constraints to loosen have to be defined.
    make_suggestions : bool
        If True and no valid solution can be found, the constraint solver
        tries to make suggestions on how to modify the constraints to find a solution.
    ...
    NoSolutionFoundError
        An invalid value encountered for the supplied constraint values or in the solution.
    """

    # Prepare input arguments and check validity
    user_constraints: Dict[str, Union[float, int, str]] = {k: v for k,v in constraints.items() if v is not None}
    _validate_args(user_constraints, loose_params)
    all_constraints: List[BoolRef] = _get_constraints(user_constraints)

    # Check if system is overconstrained and suggest to remove params in that case
    if _no_solution_exists(all_constraints):
        if make_suggestions:
            suggested_removed_params = _suggest_removed_params(user_constraints, loose_params)
            raise NoSolutionFoundError(all_constraints, suggested_removed_params=suggested_removed_params)
        raise NoSolutionFoundError(all_constraints)

    # Find unique model of the system or suggest additional params if not existent
    model = _get_unique_model_if_exists(all_constraints, user_constraints, make_suggestions)

    # Return model if n supplied by user or the found n is a valid n (even/power of two)
    current_n = _z3_to_float(model[Real("n")])
    if isinstance(user_constraints["n"], (int, np.integer)) or current_n == _round_up_n(current_n, user_constraints["n"]): # type: ignore
    ...
    if len(loose_params) == 0:
        if make_suggestions:
            ...
            raise NoSolutionFoundError(all_constraints, suggested_loose_params=suggested_loose_params)
        raise NoSolutionFoundError(all_constraints)

    # Perform distributed n widening over all loose param groups to reach next valid n
    model = _perform_n_widening_all(user_constraints, loose_params, model=model, make_suggestions=make_suggestions)
...
def _get_constraints(
    ...
    If the user supplies a number for n through user_constraints, check whether it is odd or even
    and choose constraints accordingly. If no numerical value for n is defined, choose even constraints.

    Features for rounding to next valid n (n widening):
        1) If loose params are supplied, use the respective constraint values as bounds.
        2) During n-widening, a previous_model can be supplied which overwrites the user_constraints.
        3) Additionally, during the n widening process, a different widening_n as well as
        extra add_constraints can be supplied. The add_constraint are used to ensure
        symmetric widening to interdependent parameters, e.g., pos_min and pos_max.
    """
    ...
    if isinstance(user_constraints["n"], (int, np.integer)):
        if widening_n is not None:
            # If explicit n is given, we are in the process of n widening where we always use the
            # constraints for even n. In this case, the n supplied by widening_n is usually not an
            # integer but a float. Still, we set n_is_even to True here since we round up to an even n.
            n_is_even: bool = True
        else:
    ...
    # Because the defined minimum, maximum and middle/offset values should be actual
    # grid points, there are two different constraint systems for even and odd n
    if n_is_even:
        all_constraints += z3_vars["pos_middle"] == z3_vars["pos_min"] + z3_vars["d_pos"] * z3_vars["n"] / 2,
        all_constraints += z3_vars["freq_middle"] == z3_vars["freq_min"] + z3_vars["d_freq"] * z3_vars["n"] / 2,
    else:
        all_constraints += z3_vars["pos_middle"] == z3_vars["pos_min"] + z3_vars["d_pos"] * (z3_vars["n"]-1) / 2,
        all_constraints += z3_vars["freq_middle"] == z3_vars["freq_min"] + z3_vars["d_freq"] * (z3_vars["n"]-1) / 2,

    if add_constraints is not None:
        all_constraints += [add_constraints]

    # If previous model and loose params are supplied, use these values instead of the user_constraints
    if previous_model:
        standard_values = {param: previous_model[z3_vars[param]] for param in user_constraints}
    else:
        standard_values = user_constraints.copy()

    # During n widening it is required to reach an explicit n which is therefore overwritten here
    if widening_n is not None:
...
def _validate_args(
    ...
    """
    Check supplied values for consistency and general validity.
    If the values are provided as jax.numpy.DeviceArray these are converted to normal
    scalar values. Additionally, possible duplicates in loose_params are removed mutably.
    Apart from these mutable corrections, if check is successful, do nothing, otherwise raise an error.
    """
    ...
    for var, val in user_constraints.items():
        ...
        if not (var == 'n' and isinstance(val, str)):
            if (isinstance(val, bool) or not
                ...
                raise ConstraintValueError(f"Supplied constraint ({val}) for {var} is not a real number. \
    ...
    if "n" not in user_constraints:
        raise ConstraintSolverError('There is neither an explicit numerical value for n defined, nor one of the ' +
            'rounding modes ("even" or "power_of_two") is chosen. Please supply one of these.')
    elif isinstance(user_constraints["n"], (int, np.integer)):
        if len(loose_params) != 0:
            raise ConstraintSolverError("It is not supported to supply loose params with explicitly defined n. " +
                "The loose params are only used to find a valid n but intentionally not for making overconstrained systems solvable.")
        if user_constraints["n"] < 1:
            raise ConstraintValueError(f"Supplied constraint ({user_constraints['n']}) for n should be at least 1.")
    elif isinstance(user_constraints["n"], str):
        if user_constraints["n"] not in ["even", "power_of_two"]:
            raise ConstraintValueError(f'The available rounding modes for n are: "even", "power_of_two". ' +
                f'The supplied value "{user_constraints["n"]}" is not valid.')
    else:
        raise ConstraintValueError(f"Supplied constraint ({user_constraints['n']}) for n should be of type int or str. " +
            f'The supplied value is of type {type(user_constraints)}. Please set an explicit numerical value for n (int) or ' +
            'choose one of the rounding modes ("even" or "power_of_two")')
    ...
    for loose_param in loose_params:
        if not loose_param in user_constraints:
            raise ConstraintSolverError("You can only define a used constraint as a loose_param.")
        if loose_param == "freq_middle":
            raise ConstraintSolverError("The frequency offset cannot be a loose_param.")
        if loose_param == "pos_middle":
            raise ConstraintSolverError("The position of the center of position space ('pos_middle') cannot be a loose_param.")
...
def _get_unique_model_if_exists(
    ...
    If no unique model exists raise a NoUniqueSolutionError and suggest additional
    constraints if make_suggestions is True.
    ...
    if s.check() == CheckSatResult(Z3_L_TRUE):
        ...
        different_sol_constraints = [Real(var_name) != model[Real(var_name)] for
                                     var_name in VARS_WITH_PROPS if (not var_name in user_constraints or
                                     isinstance(user_constraints[var_name], str))]
        ...
        if s.check() == CheckSatResult(Z3_L_TRUE):
            if make_suggestions:
                ...
                raise NoUniqueSolutionError(constraints, suggested_additional_params=suggested_additional_params)
...
def _perform_n_widening_all(
    ...
    # Try to improve the loose params of one group by moving a step closer to the target n
    for i, loose_param_group in enumerate(grouped_loose_params):
        group_constraint = _make_constraint_for_loose_param_group(loose_param_group, user_constraints)
        current_n = n_invalid + n_budget_per_step * (i+1) if i+1 < len(loose_params) else n_target
        # Define constraints for the next widening step using the values from the
        # previous model and the current_n which apprpoaches the target n step by step
        all_constraints = _get_constraints(user_constraints, loose_param_group, previous_model=model,
                                       widening_n=current_n, add_constraints=group_constraint)
        try:
            model = _get_unique_model_if_exists(all_constraints, user_constraints, make_suggestions=False)
        except ConstraintSolverError:
            original_constraints = _get_constraints(user_constraints, loose_params)
            if make_suggestions:
                suggested_loose_params = _suggest_loose_params(user_constraints)
                raise NoSolutionFoundError(original_constraints, suggested_loose_params=suggested_loose_params)
...
def _group_loose_params(
    ...
    """
    There are different dependencies of parameters in the FFT constraint system which
    allow sorting those into sets of independent groups such that these can be used
    for the n widening or for finding suggestions of loose params for the user.
    This method fulfills two purposes: it is mainly used to group user-provided loose params.
    By providing the user defined parameters through the loose_params argument and setting
    find_suggestions to True, this method can suggest loose_params that possibly
    match the user's constraints.
    """

    if find_suggestions:
        groups = [["d_freq"], ["d_pos"], ["freq_min", "freq_max"], ["freq_extent"], ["pos_min", "pos_max"], ["pos_extent"]]
    else:
        groups = [["d_freq"], ["d_pos"], ["freq_min", "freq_max", "freq_extent"], ["pos_min", "pos_max", "pos_extent"]]
    grouped_loose_params = []
    for group in groups:
        # If one or multiple params of one group exist, add those to grouped_loose_params
...
def _make_constraint_for_loose_param_group(
    ...
    we want to widen "*_min" and "*_max" symmetrically if both are named as loose params.
    Therefore we create an additional constraint ensuring that here.
    """

    space = "freq" if any(["freq" in param for param in loose_param_group]) else "pos"
    if any("min" in param for param in loose_param_group) and any("max" in param for param in loose_param_group):
        return user_constraints[f"{space}_min"] - Real(f"{space}_min") == Real(f"{space}_max") - user_constraints[f"{space}_max"]
    return None

def _round_up_n(current_n: float, rounding_mode: str) -> int:
    """
    If n is not explicitly defined the constraint solver, in general, finds a non-int value.
    Starting from that as the current_n, this method rounds up to the next even number or power of two.
    """
    ...
    try:
        ...
        raise ConstraintValueError(f"The above error occured while trying to round up n={current_n} to the next valid integer.") from e
...
def _z3_to_float(num: Union[RatNumRef, AlgebraicNumRef]) -> float:
    ...
    try:
        ...
        raise ConstraintValueError(f"Constraint solver results in too large value for some parameter: {overflow_approx:.2e}.")
...
def round_up_to_next_power_of_two(value: Union[int, float]) -> int:
    """Finds the next integer power of two larger than or equal to supplied value.
...
def _check_overflow(var: str, val: Union[float, int]) -> None:
    ...
    ConstraintValueError
        ...
        if `val` is an `int` and not in the range `[-sys.maxsize-1, sys.maxsize]` or
        if `val` is a `float` and its absolute value is not in the range `[0, sys.float_info.max]` and not `0.0`
    """
...
def _suggest_loose_params(
    ...
    groups = _group_loose_params(list(user_constraints.keys()), find_suggestions=True)
...
def _suggest_additional_params(
    ...
    """
    Find suggestions for additional parameters which could yield a unique solution.
    The strategy is to understand which of the required groups is missing and
    to suggest those. However, no explicit tests are performed.
    """
