...
def elementwise_two_operands(
    ...
    def fun(x1, x2, /) -> FFTArray:
        ...
        if is_on_self:
            op_norm = lambda x1, x2: getattr(x1, name)(x2)
        else:
...
class FFTArray:
    ...
    # This must be Array API compatible, so if the base library is not,
    # this is wrapped by array_api_compat.array_namespace
    _xp: Any
    ...
    # Ignore typing because they are typed in Python as returning a single bool
    # while their actual implementation is vectorized on the elements.
