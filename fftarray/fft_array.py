...
def elementwise_one_operand(name: str) -> Callable[[FFTArray], FFTArray]:
    def single_element_ufunc(inp: FFTArray) -> FFTArray:
        ...
        return FFTArray(
            ...
            dims=inp.dims,
            eager=inp.eager,
            factors_applied=tuple(True for _ in range(len(inp.dims))),
            backend=inp.backend,
        )
...
def clip(x, *, min=None, max=None) -> FFTArray:
    ...
    return FFTArray(
        ...
        dims=x.dims,
        eager=x.eager,
        factors_applied=tuple(True for _ in range(len(x.dims))),
        backend=x.backend,
    )
...
def abs(x):
    ...
    return FFTArray(
        ...
        dims=x.dims,
        eager=x.eager,
        factors_applied=tuple(True for _ in range(len(x.space))),
        backend=x.backend,
    )
...
class FFTArray:
    ...
    def __getitem__(
        ...
        return FFTArray(
            ...
            space=self.space,
            eager=self.eager,
            factors_applied=tuple(True for _ in range(len(new_dims))),
