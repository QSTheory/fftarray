...
def as_xr_pos(arr: FFTArray) -> xr.DataArray:
    return xr.DataArray(
        ...
        coords = {dim.name: np.array(dim.fft_array(space="pos")) for dim in arr.dims},
...
def as_xr_freq(arr: FFTArray) -> xr.DataArray:
    return xr.DataArray(
        ...
        coords = {dim.name: np.array(dim.fft_array(space="freq")) for dim in arr.dims},
        # attrs = _xr_attribs(arr),
    )
...
def as_xr_dataset(arr: FFTArray) -> xr.Dataset:
    return xr.Dataset({
            "pos": xr.DataArray(
                np.array(arr.into(space="pos")),
                coords = {
                    f"{dim.name}_pos": np.array(dim.fft_array(space="pos"))
                    for dim in arr.dims
                }
            ),
            "freq":  xr.DataArray(
                np.array(arr.into(space="freq")),
                coords = {
                    f"{dim.name}_freq": np.array(dim.fft_array(space="freq"))
                    for dim in arr.dims
                }
            ),
        },
