...
def sum(
    ...
    reduced_values = x.backend.numpy_ufuncs.sum(x.values(space=x.space), axis=tuple(res_meta.axis), dtype=dtype)
...
def max(
    ...
    if dim is None:
        return x.backend.numpy_ufuncs.max(x.values(space=x.space))

    res_meta = _named_dims_to_axis(x, dim)

    reduced_values = x.backend.numpy_ufuncs.max(x.values(space=x.space), axis=tuple(res_meta.axis))
...
def integrate(
    ...
    integration_element = x._backend.reduce_multiply(
        x._backend.array([fft_dim.d_pos for fft_dim in x._dims])
    )
    reduced_values = x.backend.numpy_ufuncs.sum(x.values(space=x.space), axis=tuple(res_meta.axis), dtype=dtype)
